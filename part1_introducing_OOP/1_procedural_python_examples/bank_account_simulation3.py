'''
    ...
    Bank Account Simulation 3 - Two Accounts
        This version of the bank simulation program uses the same
        approach as the version before but adds the ability to have two
        accounts.
...
while True:
    ...
    if action == 'b':
        ...
        userStartingAmount = input('How much money to have to start you\
                                   account with? ')
...
hand quickly. First, we set three global variables for each account.
Also, every function now has an if statement to choose which set
of global variables to access or change. Any time we want to add another
account, we'll need to add another set of global variables and more if
statements in every function. This is simply not a feasible approach. We need a
different way to handle an arbitrary number of accounts.
